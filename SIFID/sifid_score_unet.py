...
parser.add_argument('--path2real',
                    # default='/home/lixiang/lx/SAND-pytorvh-master/output_apd/eSPADE_Lgeo_Lsem_Lidn_2.2/x',
                    default='/data/yifan/data/CU_AR_XM0/test/sketch',
                    type=str,help=('Path to the real images'))
parser.add_argument('--path2fake',
                    # default='/home/lixiang/lx/SAND-pytorvh-master/result/output/apd_x_x/sketch',
                    default='/data/yifan/SPADE_result/test_6.5_374',
                    type=str, help=('Path to generated images'))
parser.add_argument('-c', '--gpu', default='2', type=str, help='GPU to use (leave blank for CPU only)')
parser.add_argument('--images_suffix', default='jpg', type=str, help='image file suffix')

def process_img(fineSize,img, i, j, padding):
    img = padding(img)
    img = img.crop((j, i, j + fineSize, i + fineSize))
...
def get_activations(files, model, batch_size=1, dims=64,
    ...
    # images = np.array([imread(str(f), pilmode='L').astype(np.float32)
    #                    for f in files[start:end]])
    images = np.array(torch.tensor([imread(str(fn), pilmode='L').astype(np.float32) for fn in files]).unsqueeze(-1))
    # print(images.shape) # (1,250,200,1)
    images = images.transpose((0, 3, 1, 2))
    ##the same size of every pic
    # print(images.shape) #(1,1,250,200)
    newimgs = np.array(torch.zeros(images.shape[0], 1, 256, 256))
    # print(newimgs.shape)#(1,1,256,256)
    for k in range(images.shape[0]):
        img = np.array(torch.tensor(images[k]).squeeze(0))
        # print(img.shape) # (1,250,200)
        ...
        w = 200
        h = 250
        pading_w = (loadSize - w) / 2
        pading_h = (loadSize - h) / 2
        padding = transforms.Pad((int(pading_w), int(pading_h)), fill=0, padding_mode='constant')
        i = random.randint(0, loadSize - fineSize)
        j = random.randint(0, loadSize - fineSize)
        img = process_img(256, img, i, j, padding)
        img = np.array(img)
    ...
    for i in tqdm(range(n_batches)):
        ...
        batch = torch.from_numpy(newimgs[start:end]).type(torch.FloatTensor)
        if cuda:
            batch = batch.cuda()

        pred = model(batch)#[0]
        print(pred.shape)#(1,128,128,128)
        ...
        # print(pred_arr.shape)#(1,128)
        # print((pred.cpu().data.numpy().transpose(0, 2, 3, 1).reshape(batch_size * pred.shape[2] * pred.shape[3], -1)).shape)#(16384,128)
...
def calculate_sifid_given_paths(path1, path2, batch_size, cuda, dims, suffix):
    ...
    model = unet('/home/yifan/yf/pix2pix/net_D_ins800.pth').cuda()
    if cuda:
        model.cuda()
    if device == 'cuda':
...
if __name__ == '__main__':
    ...
    sifid_values = calculate_sifid_given_paths(path1, path2, 1, args.gpu != '', 128, suffix)
